...
def process_sprite_group(sprite, canvas):
    ...
    for sprite in missile_group:
        group_m = missile_group.copy()
        remove_missile = sprite.update()
        if remove_missile:
            group_m.remove(sprite)
        
        sprite.draw(canvas)
        
def group_collide(group, other_object):
    #create a copy of set to iterate over
    set(group)
    group_r = group.copy()
    for sprite in group_r:
...
def draw(canvas):
    global time, started, lives, score, collisions
    ...
    if lives <= 0:
        ...
        for item in missile_group:
            missile_group.remove(item)
